...
namespace MischiefFramework.World.PlayerX {
    ...
    /// </summary>
    internal class SimpleCharacterController : Updateable, IEndOfTimeStepUpdateable {
        /// <summary>
        ...
        public SimpleCharacterController(Vector3 position, float characterHeight, float characterWidth, float supportHeight, float mass, bool isMainPlayer) {
            ...
            if (isMainPlayer) {
                meshObj = new SimpleCharacterRenderer(Body); 
        ...
        void IEndOfTimeStepUpdateable.Update(float dt) {
            ...
            Speed = Math.Max(MAX_SPEED, MIN_SPEED);
            Acceleration = Math.Max(MAX_ACCELERATION, MIN_ACCELERATION);
        ...
        private void HandleHorizontalMotion(Vector3 supportLocationVelocity, Vector3 supportNormal, float dt) {
            if (HasTraction && MovementDirection != Vector2.Zero) {
                ...
                if (netZVelocity > Speed) {
                    //Decelerate
                    velocityChange = Math.Min(dt * TractionDeceleration, netZVelocity - Speed);
                    Body.LinearVelocity -= velocityChange * z;
                } else {
                    //Accelerate
                    velocityChange = Math.Min(dt * Acceleration, Speed - netZVelocity);
                    Body.LinearVelocity += velocityChange * z;
                }
            } else {
                float deceleration;
